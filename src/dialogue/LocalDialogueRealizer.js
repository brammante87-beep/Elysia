...
export class LocalDialogueRealizer {
  ...
  REQUEST_SETTLEMENT_AID(i) { return `Vengo dal nostro insediamento. Abbiamo quasi finito ${this.resource(i.resource)}. Potete aiutarci?`; }
  COMMENT_ON_SETTLEMENT() { return 'Non ero mai stato qui. Da noi le cose si fanno diversamente.'; }
  ...
  customMeaning(custom, beast=false) { return ({ SHARE_FOOD_IN_CRISIS:beast?'Nel branco il cibo si divide quando manca.':'Qui nessuno resta senza cibo.', WELCOME_TRAVELERS:beast?'Il branco accoglie chi cerca riparo.':'Da noi i nuovi arrivati sono sempre accolti.', DEFEND_SETTLEMENT:'Quando arrivano i nemici, combattiamo.', PROTECT_CHILDREN_FIRST:'Durante il pericolo proteggiamo prima i bambini.', AVOID_THEFT:'Nel nostro villaggio nessuno dovrebbe rubare.', HELP_INJURED:'Accompagniamo a casa chi è ferito.', RETURN_HOME_EARLY_DURING_DANGER:'Quando c’è pericolo torniamo presto a casa.' })[custom] ?? 'Qui ci aiutiamo a modo nostro.'; }
  EXPRESS_CUSTOM(i,c) { return this.customMeaning(i.custom,c.beast); }
  PRAISE_CUSTOM(i,c) { if(i.custom==='SHARE_FOOD_IN_CRISIS')return 'Ne abbiamo abbastanza. Portiamone a loro.'; return this.customMeaning(i.custom,c.beast); }
  CRITICIZE_CUSTOM(i) { if(i.custom==='SHARE_FOOD_IN_CRISIS')return 'Non capisco perché dovremmo dividere tutto.'; if(i.custom==='AVOID_THEFT')return 'A volte si prende perché non c’è alternativa.'; return 'Non sono sicuro che questo sia il modo giusto.'; }
  TEACH_SOCIAL_NORM(i,c) { return this.customMeaning(i.custom,c.beast); }
  QUESTION_SOCIAL_NORM(i) { return i.custom==='SHARE_FOOD_IN_CRISIS'?'Perché qui condividiamo sempre il cibo?':i.custom==='WELCOME_TRAVELERS'?'Perché accogliamo sempre i nuovi arrivati?':'Perché qui facciamo così?'; }
  COMPARE_SETTLEMENT() { return 'Qui fanno le cose in modo diverso.'; }
  PRAISE_CHARACTER_REPUTATION(i,c) { return `Puoi fidarti di ${this.name(i.subjectId,c)}.`; }
  WARN_ABOUT_CHARACTER(i,c) { return i.reputation==='THIEF'?`${this.name(i.subjectId,c)} prende sempre quello che non è suo.`:`Fai attenzione a ${this.name(i.subjectId,c)}.`; }
