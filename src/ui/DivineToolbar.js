...
export class DivineToolbar {
  ...
  show(root, worldType, miracles, onSelect, onFocus) {
    ...
    if (this.presentation?.id === 'mobile') { const focus = this.document.createElement('button'); focus.type = 'button'; focus.className = 'camera-focus'; focus.textContent = '◎'; focus.title = 'Centra sul Prescelto'; focus.setAttribute('aria-label', focus.title); focus.addEventListener('click', () => onFocus?.()); this.element.append(focus); }
