...
export class Hut extends WorldEntity {
  constructor(data) { super({ collisionRadius: 1.25, kind: 'hut', ...data }); this.completed = data.completed ?? true; this.buildProgress = data.buildProgress ?? 1; this.visualVariant = data.visualVariant ?? 'hut'; this.householdId = data.householdId ?? null; this.settlementId = data.settlementId ?? null; this.occupancy = data.occupancy ?? (this.householdId ? 'occupied' : 'vacant'); this.storage = new ResourceStorage(data.storage?.values ?? data.storage ?? {}, data.storage?.capacity ?? 6); this.placementBounds = { ...(data.placementBounds ?? BuildingFootprint.dwelling()) }; this.interactionBounds = { ...(data.interactionBounds ?? BuildingFootprint.interaction(this.placementBounds)) }; }
