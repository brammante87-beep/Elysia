...
export class Renderer {
  ...
  drawMiracleEffects() { for (const effect of this.world.effects) { if (effect.type === 'intimacy') { this.drawIntimacyHearts(effect); continue; } if (effect.type === 'newLife') { this.drawNewLifeEffect(effect); continue; } const point = this.worldPoint(effect.position); const radius = (12 + effect.age * 35) * (this.canvas.width / 1200); this.context.save(); this.context.globalAlpha = 1 - effect.age / .8; this.context.strokeStyle = effect.type === 'valid' ? '#fff1a8' : '#ef8d79'; this.context.lineWidth = 3; this.context.beginPath(); this.context.arc(point.x, point.y, radius, 0, Math.PI * 2); this.context.stroke(); this.context.restore(); } }
