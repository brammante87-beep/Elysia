...
export class Renderer {
  ...
  drawHomeInfo() { if (this.world.selectedBuildingId !== this.world.hut?.id) return; const household=this.world.households.find(item=>item.homeBuildingId===this.world.hut.id); if (!household) return; const members=household.memberIds.map(id=>this.world.characters.find(character=>character.id===id)).filter(character=>character?.alive); const lines=[this.world.hut.visualVariant?.includes('den')?'Tana familiare':'Casa familiare','Abitanti:',...members.map(member=>`${member.name}${member.lifeStage==='child'?' — bambino':''}`),`Legno ${this.world.hut.storage.get('wood')}/6 · Acqua ${this.world.hut.storage.get('water')}/6 · Cibo ${this.world.hut.storage.get('food')}/6`]; const mealLabel={eating:'Stanno mangiando',mealFailedWaitingForNight:'Attendono la notte',mealCalled:'Preparano il pasto',announcing:'Il pasto è pronto'}[this.world.meals?.state]; if (mealLabel) lines.push(mealLabel); const context=this.context; context.save(); context.fillStyle='rgba(7,14,20,.92)'; context.strokeStyle='#b99d68'; context.fillRect(18,18,330,24+lines.length*20); context.strokeRect(18,18,330,24+lines.length*20); context.fillStyle='#f4e7bd'; context.textAlign='left'; lines.forEach((line,index)=>{context.font=index===0?'600 17px Georgia':'12px system-ui';context.fillText(line,34,46+index*20);}); context.restore(); }
