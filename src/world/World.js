...
import { DevelopmentDiagnostics } from '../core/DevelopmentDiagnostics.js';
import { SettlementHomePlacementSystem } from '../settlements/SettlementHomePlacementSystem.js';
import { ExaltedFoundingSystem } from '../settlements/ExaltedFoundingSystem.js';
import { MigrationNarrative } from '../data/MigrationNarrative.js';
import { FirstRivalAttack } from '../events/FirstRivalAttack.js';
import { InvasionSystem } from '../events/InvasionSystem.js';
import { ProjectileSystem } from '../combat/ProjectileSystem.js';
import { CombatSystem } from '../combat/CombatSystem.js';
import { CharacterDialogueSystem } from '../dialogue/CharacterDialogueSystem.js';
import { CommunicationIntent } from '../dialogue/CommunicationIntent.js';
import { WorldEventSystem } from '../events/WorldEventSystem.js';
import { BeliefSystem } from '../beliefs/BeliefSystem.js';
import { SettlementBeliefSummary } from '../beliefs/SettlementBeliefSummary.js';
import { CultureSystem } from '../culture/CultureSystem.js';
import { CultureBehaviorSystem } from '../culture/CultureBehaviorSystem.js';
import { SettlementDiplomacySystem } from '../diplomacy/SettlementDiplomacySystem.js';
import { SettlementWaterSystem } from '../settlements/SettlementWaterSystem.js';
...
export class World {
  ...
  restore(worldType, worldSeed, chosenOne = null, data = {}) {
    ...
    this.settlementWater.migrate();
    this.cultures = new CultureSystem(this, data.cultures); this.cultureBehaviors = new CultureBehaviorSystem(this, data.cultureBehaviors);
    this.homePlacement = new SettlementHomePlacementSystem(this, data.homePlacement);
    for (const settlement of this.settlements) this.homePlacement.repairOverlaps(settlement);
    ...
    this.exaltedFounding = new ExaltedFoundingSystem(this, data.exaltedFounding);
    this.rivalAttack = new FirstRivalAttack(this, data.rivalAttack);
    this.combat = new CombatSystem(this);
    this.projectiles = new ProjectileSystem(this, data.projectiles);
    this.invasions = new InvasionSystem(this, data.invasions);
    this.migrateCivilizationData();
    if (this.settlements.length === 2 && !this.rivalAttack.firstRivalAttackStarted) this.rivalAttack.notifySettlementRegistered(this.settlements[1]);
    this.relationships = new RelationshipSystem(this, data.relationships);
    this.dialogue = new CharacterDialogueSystem(this, data.dialogue);
    this.diplomacy = new SettlementDiplomacySystem(this, data.diplomacy);
    this.events = new WorldEventSystem(this, data.events);
  ...
  resetEntities() { this.diagnostics = new DevelopmentDiagnostics(false); this.cultures = new CultureSystem(this); this.cultureBehaviors = new CultureBehaviorSystem(this); this.beliefSystem = new BeliefSystem(this); this.settlementBeliefs = new SettlementBeliefSummary(this); this.trees = []; this.waterSources = []; this.cows = []; this.animalFoodSources = []; this.flowers = []; this.appleTrees = []; this.simulationTime = 0; this.rally = null; this.hut = null; this.homes = []; this.settlements = []; this.effects = []; this.nextEntityId = 1; this.nextCharacterId = 1; this.ai = null; this.ais = new Map(); this.households = []; this.selectedCharacterId = null; this.selectedBuildingId = null; this.reservations = new TargetReservation(); this.playerCreatedTreeCount = 0; this.worldTime = new WorldTime(); this.reproduction = new ReproductionSystem(this); this.meals = new HouseholdMealSystem(this); this.settlementProgression = new SettlementProgression(this); this.homePlacement = new SettlementHomePlacementSystem(this); this.newcomers = new NewcomerSystem(this); this.exaltedFounding = new ExaltedFoundingSystem(this); this.relationships = new RelationshipSystem(this); this.householdProgression = new HouseholdProgression(this); this.plantProgression = new PlantWorldProgression(this); this.rivalAttack = new FirstRivalAttack(this); this.combat = new CombatSystem(this); this.projectiles = new ProjectileSystem(this); this.invasions = new InvasionSystem(this); this.divineTeaching = new DivineTeachingSystem(this); this.dialogue = new CharacterDialogueSystem(this); this.diplomacy = new SettlementDiplomacySystem(this); this.events = new WorldEventSystem(this); this.settlementWater = new SettlementWaterSystem(this); this.eventNotification = null; }
  get foundingReservations() { return this.exaltedFounding?.reservations ?? []; }
  migrateCivilizationData() {
    const migration = this.newcomers.narrative ?? new MigrationNarrative();
    for (const settlement of this.settlements) {
      if (settlement.centralStructureId || this.findHome(settlement.foundingHomeId)?.kind === 'centralStructure') settlement.establish();
      const founder=this.characters.find(character=>character.id===settlement.founderId);
      if(founder && settlement.id!=='settlement-1') { founder.hasFoundedSettlement=true; founder.canFoundSettlement=false; }
    }
    for (const character of this.characters) {
      character.homeSettlementId ??= character.settlementId;
      if (character.chosenOne || character.birthCycle !== null) character.originWorld = 'ELYSIA';
      else if (character.newcomer && (!character.originWorld || character.originWorld === 'ELYSIA')) {
        const seed = this.worldSeed + Number(String(character.id).match(/\d+/)?.[0] ?? 0);
        character.originWorld = migration.origin(seed); character.arrivalStoryType ??= migration.story(seed);
      }
    }
  }
  ...
  findCombatant(id) { return this.characters.find(item => item.id === id) ?? this.invasions.enemies.find(item => item.id === id) ?? null; }
  ...
  homeFor(character) { const household = this.households.find(item => item.id === character.householdId); return this.findHome(household?.homeBuildingId ?? character.homeBuildingId) ?? (!household ? this.hut : null); }
  ...
  findEdgeReachablePosition(target) { const pathfinder = new Pathfinder(); const candidates = []; for (let y = 1; y < this.terrain.height - 1; y += 1) for (let x = 1; x < this.terrain.width - 1; x += 1) { if (!this.isWalkable(x,y)) continue; const coastline = [[1,0],[-1,0],[0,1],[0,-1]].some(([dx,dy]) => !this.isWalkable(x+dx,y+dy)); if (coastline) candidates.push({x,y}); } if (!candidates.length) for (let y = 1; y < this.terrain.height - 1; y += 1) for (let x = 1; x < this.terrain.width - 1; x += 1) if (this.isWalkable(x,y) && Math.min(x,y,this.terrain.width-1-x,this.terrain.height-1-y) <= 2) candidates.push({x,y}); candidates.sort((a,b) => Math.hypot(b.x-target.x,b.y-target.y)-Math.hypot(a.x-target.x,a.y-target.y) || a.y-b.y || a.x-b.x); return candidates.find(point => pathfinder.findPath(this.terrain, point, target).length > 0) ?? null; }
  findSettlementHomePosition(settlement, ownerId = `placement-${this.nextEntityId}`) { return this.homePlacement.reserve(settlement, ownerId); }
  findSettlementFoundingPosition(origin) { const source=this.settlementWater.availableSource(origin); const position=this.settlementWater.foundingPosition(source); return position ? { ...position, waterSourceId:source.id } : null; }
  moveCharacterToHousehold(householdId, homeId, settlementId, character) { const previous = this.households.find(item => item.id === character.householdId); previous?.removeMember(character.id); const household = this.households.find(item => item.id === householdId); if (!household) return false; household.addMember(character.id); character.householdId = household.id; character.homeBuildingId = homeId; character.settlementId = settlementId; character.homeSettlementId = settlementId; return true; }
  ...
  killCharacter(character, cause = null) { if (!character?.alive) return false; const closeFamily = this.characters.filter(item => item.alive && (item.partnerId === character.id || RelationshipSystem.isCloseFamily(item, character))); const partner = this.characters.find(item => item.id === character.partnerId); if (partner?.alive) { partner.formerPartnerIds = [...new Set([...(partner.formerPartnerIds ?? []), character.id])]; partner.partnerId = null; } if (character.partnerId) character.formerPartnerIds = [...new Set([...(character.formerPartnerIds ?? []), character.partnerId])]; character.partnerId = null; character.alive = false; character.insideHome = false; this.households.find(item => item.id === character.householdId)?.removeMember(character.id); const home = this.findHome(character.homeBuildingId); const household = this.households.find(item => item.id === character.householdId); if (home) home.occupancy = household?.memberIds.length ? 'occupied' : 'vacant'; this.reproduction.endRelationshipsFor(character.id); this.reservations.releaseFor(character.id); this.ais.get(character.id)?.interrupt(); for (const survivor of closeFamily) { survivor.needs.familyEvent(false, true); survivor.memories.remember({ eventType: 'FAMILY_DEATH', participants: [character.id], cycle: this.worldTime.cycle, emotionalImpact: -1, importance: 1, knowledgeSource: 'WITNESSED', cause }); this.dialogue?.say(new CommunicationIntent({ speakerId: survivor.id, targetType: cause === 'lightning' ? 'PLAYER' : 'SELF', intent: 'EXPRESS_GRIEF', subjectId: character.id, cause, emotion: 'SAD', priority: 'CRITICAL' })); } return true; }
  ...
  update(deltaTime) { if (this.rivalAttack.update(deltaTime)) return; this.simulationTime += deltaTime; this.invasions.update(deltaTime); this.projectiles.update(deltaTime); for (const tree of this.appleTrees) tree.update(this.simulationTime); this.effects.forEach(effect => { effect.age += deltaTime; }); this.effects = this.effects.filter(effect => effect.age < (effect.duration ?? (effect.type === 'newLife' ? 6 : effect.type === 'intimacy' ? 4 : effect.type === 'rayOfLight' ? 12 : effect.type === 'settlementTransform' ? 5 : effect.type === 'transform' ? 1.4 : ['lightning','blessing','flowerGrowth','exalted','changeSex','giveWeapons','shield','theftApproval','theftDisapproval'].includes(effect.type) ? 1.6 : .8))); this.hut?.update?.(deltaTime); if (this.worldType !== WorldTypeId.PLANT) { const startCycle = this.worldTime.cycle; const startElapsed = this.worldTime.elapsed; const transitions = this.worldTime.update(deltaTime, true); for (const phase of transitions) { this.diagnostics.trace('TIME_PHASE_CHANGED', { phase, cycle: this.worldTime.cycle }); this.handleTimePhase(phase); } this.updateFlowers(); if (this.rally) { this.rally.remaining -= deltaTime; if (this.rally.remaining <= 0) { this.rally=null; for (const ai of this.ais.values()) if (ai.task === CharacterAI.Tasks.RALLY) ai.interrupt(); } } this.householdProgression.update(deltaTime); this.settlementProgression.ensureFoundingSettlement(); this.newcomers.update(deltaTime); this.exaltedFounding.update(deltaTime); this.relationships.update(deltaTime); this.diplomacy.update(deltaTime); for (const character of this.characters.filter(item => item.alive)) character.needs.update(deltaTime, { shielded: character.hasDivineShield, danger: this.invasions?.enemies?.some(enemy => enemy.alive) }); this.dialogue.update(deltaTime); this.events.update(deltaTime); this.cultures.update(deltaTime); this.cultureBehaviors.update(deltaTime); const daytimeDelta = this.worldTime.daytimeBetween(startCycle, startElapsed); this.meals.update(daytimeDelta, daytimeDelta > 0); for (const ai of this.ais.values()) if (!ai.character.foundingState && !ai.character.visitState && (this.worldTime.phase === WorldTime.Phases.NIGHT || this.householdProgression.partnerId !== ai.character.id || this.householdProgression.state === HouseholdProgression.States.COMPLETE)) ai.update(deltaTime); if (this.worldTime.phase === WorldTime.Phases.NIGHT) this.reproduction.evaluateNight(this.worldTime.cycle); } else this.plantProgression.update(deltaTime); }
  handleTimePhase(phase) { if (phase === WorldTime.Phases.NIGHT) { this.meals.beginNight(); for (const ai of this.ais.values()) if (ai.character.alive && ai.task !== CharacterAI.Tasks.RALLY) ai.returnHome(); } if (phase === WorldTime.Phases.DAWN) { this.exaltedFounding.evaluate(); this.newcomers.processCompletedDay(this.worldTime.cycle); const child = this.reproduction.birthAtDawn(this.worldTime.cycle); this.reproduction.growChildren(this.simulationTime); for (const ai of this.ais.values()) if (ai.character.alive) ai.wake(); if (child) this.ensureAI(child).wake(); } }
  toJSON() { return { simulationTime: this.simulationTime, trees: this.trees.map(e => e.toJSON()), waterSources: this.waterSources.map(e => e.toJSON()), cows: this.cows.map(e => e.toJSON()), animalFoodSources: this.animalFoodSources.map(e => e.toJSON()), flowers: this.flowers.map(e => e.toJSON()), appleTrees: this.appleTrees.map(e => e.toJSON()), hut: this.hut?.toJSON() ?? null, homes: this.homes.map(e => e.toJSON()), settlements: this.settlements.map(e => e.toJSON()), cultures: this.cultures.toJSON(), cultureBehaviors: this.cultureBehaviors.toJSON(), characters: this.characters.map(e => e.toJSON()), households: this.households.map(e => e.toJSON()), ais: Object.fromEntries([...this.ais].map(([id, ai]) => [id, ai.toJSON()])), ai: this.ai?.toJSON() ?? null, householdProgression: this.householdProgression.toJSON(), plantProgression: this.plantProgression.toJSON(), playerCreatedTreeCount: this.playerCreatedTreeCount, worldTime: this.worldTime.toJSON(), reproduction: this.reproduction.toJSON(), meals: this.meals.toJSON(), newcomers: this.newcomers.toJSON(), exaltedFounding: this.exaltedFounding.toJSON(), homePlacement: this.homePlacement.toJSON(), relationships: this.relationships.toJSON(), diplomacy: this.diplomacy.toJSON(), dialogue: this.dialogue.toJSON(), events: this.events.toJSON(), settlementProgression: this.settlementProgression.toJSON(), rivalAttack: this.rivalAttack.toJSON(), invasions: this.invasions.toJSON(), projectiles: this.projectiles.toJSON(), nextCharacterId: this.nextCharacterId }; }
}
