...
export class World {
  ...
  findSettlementFoundingPosition(origin) { const candidates = []; for (let y = 3; y < this.terrain.height - 3; y += 2) for (let x = 3; x < this.terrain.width - 3; x += 2) { const point = { x: x + .5, y: y + .5 }; if (!this.canPlace(point, 2) || this.settlements.some(settlement => Math.hypot(point.x - settlement.center.x, point.y - settlement.center.y) < Config.MIN_SETTLEMENT_DISTANCE)) continue; const path = new Pathfinder().findPath(this.terrain, origin, point); if (path.length) candidates.push({ point, distance: Math.hypot(point.x - origin.x, point.y - origin.y) }); } candidates.sort((a, b) => b.distance - a.distance); return candidates[0]?.point ?? null; }
