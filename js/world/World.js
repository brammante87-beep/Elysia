...
import { AdultCharacterGeometry } from "../renderer/AdultCharacterGeometry.js";
import { LightningEffect } from "../miracles/LightningEffect.js";
...
export class World {
    ...
    update(delta) {
        ...
        this.villagers.filter((villager) => villager.alive).forEach((villager) => {
    ...
    getHousePlateName(house) {
        const adults = this.getOrderedLivingHouseAdults(house);
        if (adults.length === 0) { return "Casa vuota"; }
        if (adults.length === 1) { return adults[0].name; }
        return `${adults[0].name} ❤️ ${adults[1].name}`;
    }
    ...
    findCompactHouseSite(builder) {
        ...
        return this.searchCompactHouseSiteInBand(builder, 90, 145, 30) ||
            this.searchCompactHouseSiteInBand(builder, 145, 210, 30) ||
            this.searchCompactHouseSiteInBand(builder, 210, 275, 40) ||
    ...
    searchDeterministicCompactHouseSite(builder) {
        ...
        for (let distance = 90; distance <= 275; distance += 10) {
    ...
    searchCompactHouseSiteInBand(builder, minDistance, maxDistance, attempts) {
        ...
        for (let attempt = 0; attempt < attempts; attempt += 1) {
            ...
            const site = { x: center.x + Math.cos(angle) * distance, y: center.y + Math.sin(angle) * distance };
            if (!this.isValidCompactHouseSite(site.x, site.y, builder)) { continue; }
            const neighborDistance = Math.min(...this.houses.map((house) => Math.hypot(house.x - site.x, house.y - site.y)));
            if (neighborDistance < bestNeighborDistance) { bestSite = site; bestNeighborDistance = neighborDistance; }
        }
        return bestSite;
    }
    ...
    isValidCompactHouseSite(x, y, builder) {
        ...
        return distance !== null && distance <= 275 && this.canPlaceAutonomousHouseAt(x, y, builder);
    }
    ...
    canPlaceAutonomousHouseAt(x, y, builder) {
        ...
        return x - house.radius >= 0 && y - house.radius >= 0 &&
            ...
            this.houses.every((existingHouse) => Math.hypot(existingHouse.x - x, existingHouse.y - y) >= 100) &&
    ...
    selectHouse(house) {
        if (house !== null && this.houses.includes(house)) { this.selectedHouse = house; }
    }

    getSelectedHouse() {
        if (this.selectedHouse !== null && this.houses.includes(this.selectedHouse)) { return this.selectedHouse; }
        this.selectedHouse = null;
        return this.getChosenHouse();
    }

    isHouseSpriteAtPosition(house, x, y) {
        const halfWidth = house.upgraded ? 41 : 35;
        const roofTop = house.upgraded ? 40 : 35;
        return x >= house.x - halfWidth && x <= house.x + halfWidth && y >= house.y - roofTop && y <= house.y + 34;
    }

    isHouseLabelAtPosition(house, x, y) {
        const halfWidth = Math.max(28, this.getHouseName(house).length * 4.2);
        return x >= house.x - halfWidth && x <= house.x + halfWidth && y >= house.y - 56 && y <= house.y - 38;
    }
    ...
    getHouseHudSummary(house) {
        ...
        return { title: house === null ? "Nessuna casa" : this.getHouseName(house), totals, capacity: house?.capacity || 0, occupants: this.getHouseOccupantSummaries(house), cost, phaseText, lastNightText: this.getLastNightText(house) };
    }

    getHouseOccupantSummaries(house) {
        if (house === null) { return []; }
        return this.getLivingHouseResidents(house).map((person) => {
            if (!person.isAdult) { return `👶 ${person.name}`; }
            if (person !== house.owner) { return `❤️ ${person.name}`; }
            return person.name;
        });
    }
    ...
    updateHero(delta) {
        if (this.hero.targetMushroom !== null) { this.updateHeroMushroomCollector(delta); return; }
    ...
    updateHeroMushroomCollector(delta) {
        const mushroom = this.hero.targetMushroom;
        if (!this.mushrooms.includes(mushroom) || !mushroom.alive) { this.hero.targetMushroom = null; this.hero.state = "idle"; return; }
        const distance = Math.hypot(mushroom.x - this.hero.x, mushroom.y - this.hero.y);
        const stopDistance = this.hero.radius + mushroom.radius + 3;
        if (distance > stopDistance + 0.5) {
            const step = Math.min(this.hero.speed * delta, distance - stopDistance);
            this.hero.x += ((mushroom.x - this.hero.x) / distance) * step;
            this.hero.y += ((mushroom.y - this.hero.y) / distance) * step;
            this.hero.state = "walking"; return;
        }
        this.hero.meat += mushroom.foodValue;
        this.removeMushroom(mushroom);
        this.hero.targetMushroom = null; this.hero.state = "idle";
        this.feedbackMessages.push({ text: "Fungo raccolto", timer: 3 });
        this.requestAutosave();
    }
    ...
    updateVillager(villager, delta) {
        ...
        if (villager.destination === null) {
            ...
            if (villager.idleTimer <= 0) {
                villager.destination = this.getIdleVillagerDestination();
    ...
    getIdleVillagerDestination() {
        if (this.villageTransformationCompleted && this.villageBounds && Math.random() < 0.3) {
            const angle = Math.random() * Math.PI * 2;
            const radius = 45 + Math.random() * 55;
            return { x: this.villageBounds.centerX + Math.cos(angle) * radius, y: this.villageBounds.centerY + Math.sin(angle) * radius };
        }
        return this.terrain.getRandomWalkableWorldPosition();
    }
    ...
    clearPersonResourceWork(person) {
        ...
        if (person === this.hero) { this.clearHeroMushroomWork(); }
        person.destination = null;
        if (person === this.hero) { this.heroDestination = null; }
    }
    ...
    isAdultCharacterSpriteAtPosition(character, x, y) {
        const bounds = AdultCharacterGeometry.getAdultCharacterBounds(character);
    ...
    getHouseAtWorldPosition(x, y) {
        return [...this.houses].reverse().find((house) => this.isHouseSpriteAtPosition(house, x, y) || this.isHouseLabelAtPosition(house, x, y)) || null;
    }
    ...
    removePreVillageAnimals() {
        this.clearHeroAnimalWork();
        this.villagers.forEach((villager) => {
            if (villager.targetAnimal !== null) { this.clearVillagerAnimalWork(villager); }
        });
        this.animals.forEach((animal) => {
            animal.assignedHero = null; animal.assignedVillager = null; animal.isBeingHunted = false;
        });
        this.animals = [];
    }

    getLightningTargetAt(x, y) {
        this.debugLightning("Lightning target search started", { x, y });
        const target = [...this.villagers].reverse().find((villager) => villager.alive && villager.isAdult && this.isAdultCharacterSpriteAtPosition(villager, x, y)) || null;
        this.debugLightning(target ? "Lightning target found" : "Lightning target not found", target ? { targetId: target.id } : null);
        return target;
    }

    strikeWithLightningAt(x, y) {
        const target = this.getLightningTargetAt(x, y);
        if (target === null) {
            this.lastLightningResult = { success: false, targetId: null, reason: "no-target" };
            this.feedbackMessages.push({ text: "Nessun abitante da colpire", timer: 3 });
            return { success: false, target: null, reason: "no-target" };
        }
        if (!target.receiveLightningStrike(this)) {
            this.lastLightningResult = { success: false, targetId: null, reason: "no-target" };
            return { success: false, target: null, reason: "no-target" };
        }
        this.debugLightning("Lightning strike applied", { targetId: target.id });
        this.lightningEffects.push(new LightningEffect(target.x, target.y));
        this.debugLightning("Lightning effect created", { targetId: target.id });
        this.feedbackMessages.push({ text: "Il tuo dio ti ammonisce", timer: 3 });
        this.lastLightningResult = { success: true, targetId: target.id, reason: null };
        return { success: true, target, reason: null };
    }

    killVillager(villager) {
        this.clearVillagerAllWork(villager);
        villager.alive = false; villager.destination = null; villager.state = "dead";
        if (villager.house) { villager.house.occupants = villager.house.occupants.filter((occupant) => occupant !== villager); }
        villager.house = null;
        this.addEvent(`${villager.name} è morto dopo l'ira divina.`);
        this.feedbackMessages.push({ text: `${villager.name} è morto`, timer: 4 });
    }

    getRaiderAtWorldPosition(x, y) {
        return this.raiders.find((raider) => raider.alive !== false && Math.hypot(raider.x - x, raider.y - y) <= (raider.radius || 14)) || null;
    }

    killRaider(raider) {
        raider.alive = false;
        if (typeof raider.releaseTarget === "function") { raider.releaseTarget(); }
        this.raiders = this.raiders.filter((candidate) => candidate !== raider);
        this.feedbackMessages.push({ text: "Il predone è stato folgorato", timer: 3 });
    }

    updateLightningEffects(delta) {
        this.lightningEffects.forEach((effect) => effect.update(delta));
        this.lightningEffects = this.lightningEffects.filter((effect) => effect.alive);
    }
    ...
    placeFlowerAt(x, y) {
        const rejection = this.validateFlowerPosition(x, y);
        this.lastFlowerPlacementReason = rejection.reason;
        this.debugFlower("Flower validation result", rejection);
        if (!rejection.success) {
            this.feedbackMessages.push({ text: "Qui il fiore non può crescere", timer: 3 });
            return { success: false, flower: null, reason: rejection.reason };
        }
        ...
        return { success: true, flower, reason: null };
    }

    placeMushroomAt(x, y) {
        const rejection = this.validateMushroomPosition(x, y);
        if (!rejection.success) { this.feedbackMessages.push({ text: "Qui il fungo non può crescere", timer: 3 }); return { success: false, mushroom: null, reason: rejection.reason }; }
        const mushroom = new Mushroom(x, y); this.assignResourceId(mushroom); this.mushrooms.push(mushroom);
        this.feedbackMessages.push({ text: "Un fungo è spuntato", timer: 3 }); this.requestAutosave();
        return { success: true, mushroom, reason: null };
    }

    validateMushroomPosition(x, y) {
        if (!Number.isFinite(x) || !Number.isFinite(y) || !this.contains(x, y)) { return { success: false, reason: "terrain" }; }
        if (!this.terrain.isGrassAtWorldPosition(x, y)) { return { success: false, reason: "terrain" }; }
        const mushroom = new Mushroom(x, y);
        const blocked = this.overlapsAnyHouse(mushroom) || this.overlapsAnyTree(mushroom) || this.overlapsAnyWaterSource(mushroom) || this.overlapsAnyAnimal(mushroom) ||
            this.overlapsAnyFruitTree(mushroom) || this.overlapsAnyFlower(mushroom) || this.overlapsAnyMushroom(mushroom) ||
            (this.villageWell && this.overlapsEntity(mushroom, this.villageWell)) || (this.hero && this.overlapsEntity(mushroom, this.hero)) ||
            this.villagers.some((villager) => villager.alive && this.overlapsEntity(mushroom, villager)) || this.overlapsVillageWallOrGate(mushroom);
        return blocked ? { success: false, reason: "occupied" } : { success: true, reason: null };
    }

    getMushroomAtWorldPosition(x, y) { return this.mushrooms.find((mushroom) => mushroom.alive && Math.hypot(mushroom.x - x, mushroom.y - y) <= mushroom.radius) || null; }
    getMushrooms() { return this.mushrooms; }
    removeMushroom(mushroom) { mushroom.alive = false; this.mushrooms = this.mushrooms.filter((item) => item !== mushroom); }
    overlapsAnyMushroom(entity) { return this.mushrooms.some((mushroom) => mushroom.alive && this.overlapsEntity(entity, mushroom)); }
    commandHeroToCollectMushroom(mushroom) {
        if (!this.mushrooms.includes(mushroom) || !mushroom.alive) { return false; }
        this.clearPersonResourceWork(this.hero); this.hero.targetMushroom = mushroom; this.heroDestination = null;
        this.hero.state = "walking"; this.hero.autonomousAction = false; return true;
    }

    clearHeroMushroomWork() {
        if (this.hero !== null) { this.hero.targetMushroom = null; }
    }

    validateFlowerPosition(x, y) {
        if (!Number.isFinite(x) || !Number.isFinite(y) || !this.contains(x, y)) { return { success: false, reason: "bounds" }; }
        if (!this.terrain.isWalkableAtWorldPosition(x, y) && !this.terrain.isGrassAtWorldPosition(x, y)) { return { success: false, reason: "terrain" }; }
        const flower = new Flower(x, y);
        if (this.overlapsAnyHouse(flower)) { return { success: false, reason: "House" }; }
        if (this.overlapsAnyTree(flower)) { return { success: false, reason: "Tree" }; }
        if (this.overlapsAnyWaterSource(flower)) { return { success: false, reason: "WaterSource" }; }
        if (this.overlapsAnyAnimal(flower)) { return { success: false, reason: "Animal" }; }
        if (this.villageWell && this.overlapsEntity(flower, this.villageWell)) { return { success: false, reason: "Well" }; }
        if (this.overlapsAnyFruitTree(flower)) { return { success: false, reason: "FruitTree" }; }
        if (this.overlapsAnyFlower(flower)) { return { success: false, reason: "Flower" }; }
        if (this.villagers.some((villager) => villager.alive && this.overlapsEntity(flower, villager))) { return { success: false, reason: "Villager" }; }
        if (this.hero && this.hero.alive && this.overlapsEntity(flower, this.hero)) { return { success: false, reason: "Hero" }; }
        if (this.overlapsVillageWallOrGate(flower)) { return { success: false, reason: "palisade or gate" }; }
        return { success: true, reason: null };
    }

    debugFlower(message, details) { if (globalThis.ELYSIA_DEBUG === true) { console.debug(message, details); } }
