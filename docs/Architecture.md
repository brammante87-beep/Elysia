...
## Alpha 1.8.0 — Settlement diplomacy architecture

`SettlementDiplomacySystem` is the single World-owned coordinator for timed inter-Settlement relations, requests, visits and physical deliveries. One `SettlementRelationship` exists per unordered pair and contains separate perceptions for each endpoint: trust, affinity, cooperation, tension, bounded semantic memories and hysteretic status. `SettlementVisit` and `AidDelivery` are independent persisted state machines. Character `settlementId`, Household membership and home never change during a visit; temporary visitor fields are restored from the authoritative visit. Cargo leaves donor `ResourceStorage` exactly once and a persisted `rewardApplied` guard prevents duplicate delivery gratitude. Pair migration is neutral except for verifiable founder kinship. Stale pairs are pruned safely.

The system evaluates relation maintenance periodically rather than per pair every frame; only active journeys advance continuously. Dialogue uses new semantic intents, and Renderer only realizes qualitative status and known memories. Cultural diffusion is a small post-contact signal rather than culture copying. No separate Beast diplomacy engine, government, treaty, market, territory or war system exists.


## Alpha 1.8.5 settlement boundary architecture

`SettlementWaterSystem` owns deterministic source supply, unique claims, founding-site selection and save repair. `ExaltedFoundingSystem` reserves a slot and source before travel and persists the founder’s single-use eligibility. `SettlementRules` is the shared eligibility boundary. `CharacterAI` confines resources to the Character’s home community, while `SettlementDiplomacySystem` preserves abstract relationships but cancels legacy physical traffic. Dedicated `LightningRenderer`, `DivineWeaponRenderer`, and `LionRenderer` classes keep rebuilt drawing responsibilities outside simulation.
